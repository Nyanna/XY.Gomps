...
public class StatLayer extends SimpleLayer implements ILayer {
  ...
  protected void draw(final IDataObject robj) {
    ...
    final Double lon1 = Double.valueOf(parent.getViewPort().origin.lon
        + parent.getViewPort().dimension.height * 0.02);
    final Double lat1 = Double.valueOf(parent.getViewPort().origin.lat
        + parent.getViewPort().dimension.width * 0.02);
    Double lon2 = Double.valueOf(parent.getViewPort().origin.lon
        + parent.getViewPort().dimension.height * 0.4);
    // calculate scale
    Double distance = GeoTools.getDistance(lon1.doubleValue(), lon2.doubleValue(),
        lat1.doubleValue()); // 2753m
    final double roundBy = Integer.valueOf(distance.intValue()).toString().length() > 1 ? (int) Math
        .pow(10, (Integer.valueOf(distance.intValue()).toString().length() - 1)) : 1;
    distance = Double.valueOf((Math.round(distance.doubleValue() / roundBy) * roundBy)); // 3000m
    lon2 = Double.valueOf(lon1.doubleValue()
        + GeoTools.metersToLon(distance.doubleValue(), lat1.doubleValue()));
    final double sxt = parent.getViewPort().origin.lon + parent.getViewPort().dimension.height
        * 0.04;
    ...
    listener.draw(new DrawText(lat1.doubleValue(), sxt, GeoTools.convertMeters(distance
        .doubleValue()), BASERGB));
